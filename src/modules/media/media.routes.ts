...
const mediaRoutes = {
  publicRoutes: () => {
    ...
    router.post('/aws/pre-sign', validateBody(generatePreSignedUrl), generateUploadURL);
    router.post('/', validateBody(createMediaSchema), createMedia);
    return router;
  },
  apiRoutes: () => {
    router.get('/:id', validateParams(validateId), getMediaById);
    router.post('/aws/pre-sign', validateBody(generatePreSignedUrl), generateUploadURL); // pre-sign for aws upload
