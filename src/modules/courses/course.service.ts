...
export class CourseService {
  ...
  async getAdminCourses(query: object, extractFields: string = basicFields, limit: number = 30): Promise<Partial<ICourse[]> | null> {
    try {
      return await Course.find(query, extractFields)
        .limit(limit)
        .populate([{ path: 'instructor.avatar', model: 'Media', select: mediaMinFields, strictPopulate: false }])
        .lean()
        .exec();
    } catch (error) {
      return error;
